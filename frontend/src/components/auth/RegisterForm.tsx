      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div className="w-full">
          ...
          <div className="grid grid-cols-3 gap-3">
            ...
            >
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center mx-auto mb-2 ${
                role === UserRole.ADMIN ? 'bg-indigo-100' : 'bg-slate-100'
              }`}>
                <svg className={`w-5 h-5 ${role === UserRole.ADMIN ? 'text-indigo-600' : 'text-slate-500'}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M9 12.75L11.25 15 15 9.75m-3-7.036A11.959 11.959 0 013.598 6 11.99 11.99 0 003 9.749c0 5.592 3.824 10.29 9 11.623 5.176-1.332 9-6.03 9-11.622 0-1.31-.21-2.571-.598-3.751h-.152c-3.196 0-6.1-1.248-8.25-3.285z" />
                </svg>
              </div>
              <p className={`font-semibold text-sm ${role === UserRole.ADMIN ? 'text-indigo-700' : 'text-slate-700'}`}>Admin</p>
              <p className="text-xs text-slate-500 mt-0.5">Kelola platform</p>
            </button>
          </div>
        </div>
