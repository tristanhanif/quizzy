    ...
    if (quiz && !initialized) {
      ...
      setQuestions(
        quiz.questions.map((q) => ({
          ...
          type: q.type,
          points: q.points,
          choices: q.type === QuestionType.TRUE_FALSE
            ? ['Benar', 'Salah']
            : q.choices.length > 0
              ? q.choices
              : ['', '', '', ''],
  ...
  const updateQuestion = (index: number, field: keyof QuestionInput, value: any) => {
    ...
    if (field === 'type') {
      if (value === QuestionType.TRUE_FALSE) {
        updated[index].choices = ['Benar', 'Salah'];
        if (!['Benar', 'Salah'].includes(updated[index].correctAnswer)) {
          updated[index].correctAnswer = '';
        }
      } else if (updated[index].choices.length < 4) {
    ...
    return (
      <div className="flex min-h-[400px] items-center justify-center">
        <div className="flex flex-col items-center gap-3 text-center">
          ...
          <p className="text-sm font-medium text-gray-900">Quiz tidak ditemukan</p>
          <button
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      <div>
        ...
        <p className="mt-1 text-sm text-gray-500">Perbarui informasi quiz dan pertanyaan</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  {q.choices.map((choice, cIndex) => (
                    <div key={cIndex} className="flex items-center gap-3">
                      ...
                      {q.type === QuestionType.TRUE_FALSE ? (
                        <span className="flex-1 px-4 py-2 rounded-xl border border-gray-200 bg-gray-50 text-sm text-gray-700">
                          {choice}
                        </span>
                      ) : (
                        <input
        ...
        <div className="flex gap-4 rounded-2xl border border-gray-100 bg-white p-4 shadow-sm">
          ...
          >
            {updateQuiz.isPending ? 'Menyimpan...' : 'Simpan Perubahan'}
          </button>
        </div>
      </form>
    </div>
