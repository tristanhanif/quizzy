        ...
        <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-6">
          <div>
            ...
            <p className="text-indigo-200 mt-1">
              {isCreator ? 'Kelola quiz dan pantau sesi aktifmu' : 'Siap untuk mengikuti quiz berikutnya?'}
            </p>
            {user?.displayId && (
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {isCreator ? (
          <>
            ...
            <StatCard value="0" label="Sesi Aktif" iconBg="bg-indigo-50" icon={
              <svg className="h-5 w-5 text-indigo-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                <path strokeLinecap="round" strokeLinejoin="round" d="M3.75 13.5l10.5-11.25L12 10.5h8.25L9.75 21.75 12 13.5H3.75z" />
              </svg>
            } />
            <StatCard value="0" label="Total Peserta" iconBg="bg-slate-100" icon={
              <svg className="h-5 w-5 text-slate-600" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
        ...
        {isLoading ? (
          ...
          <div className="bg-white border border-slate-200 rounded-xl p-12 text-center">
            ...
            <p className="text-sm text-slate-500 mb-5 max-w-sm mx-auto">
              {isCreator ? 'Mulai buat quiz pertamamu!' : 'Tunggu creator membuat quiz baru.'}
            </p>
            {isCreator && (
              <button
                onClick={() => router.push('/quizzes/create')}
